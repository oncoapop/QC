...
# Script is called by R-script
# #########################################
# Validation Pipeline MiSeq WSOP 2013-001 #
#####################################################################
# DO NOT CHANGE THE VALUES IN THIS SCRIPT UNLESS THE PLATE MAPS IS  #
# ALTERED                                                           #
#####################################################################

tempchk="/home/dyap/dyap_temp/"

#################################
# Enter the name of sample to run 
# name=`cat $tempchk"name" | tr " " "," | awk -F, '{print $2}' | tr -d '"' | tr "\n" "," | sed 's/^,//' | tr -d ","`
name="SA499"
################################
...
#top=`cat $tempchk"top" | tr " " "," | awk -F, '{print $2}' | tr -d '"' | tr "\n" "," | sed 's/^,//' | tr "," " "`
top="T N X2 X4 NSG X6"

# Rows I-P of 384 well plate
# bottom=`cat $tempchk"bottom" | tr " " "," | awk -F, '{print $2}' | tr -d '"' | tr "\n" "," | sed 's/^,//' | tr "," " "`
bottom="X3 X5 X6WGA"
...
###############################
# Check 
echo "These are the parameters that will run on this script:"
echo "Name of sample: "$name
echo "Samples on the top half of 384 well plate: "$top
echo "Samples on the bottom half of 384 well plate: "$bottom
echo "########################################################################"
echo "Press return to continue or Ctrl-C to exit if in correct."
read ans
###############################
...
# This calls the R-script to generate the PDFs

Rscript /home/dyap/R/R-scripts/Temp_Corr2.R --no-save --no-restore --args $name
